...
quality_check <- function(win_size, upper, dive_phase, narwhal_acc, plots = FALSE) {
  ...
  if(plots) {
    ...
    plot_path <- file.path("figures", "s2n", glue::glue())
    s2n_plot <- ggplot(narwhal_s2n$psd_df, aes(...)) +
      geom_line(...) +
      geom_area(...)
    ggsave(s2n_plot)
...
#going to want to use facet_wrap, with different plots for each win_s, upper combination 
```
