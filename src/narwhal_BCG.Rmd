...
#keep - a vector of character strings of dive phases to keep in the df. eg: c("bottom", "surface", "subsurface", "descent", "ascent")

filt_ml <- function(narwhal_id, keep) {
  read_tsv(find_motionless(narwhal_id)) %>% 
    mutate(region_id = row_number()) %>% 
    filter(all(keep %in% dive_phase) & dive_phase %in% keep) %>% 
    select(-c(duration, narwhal_id, dive_phase, notes, meets_criteria)) %>% 
    arrange(time_start) %>% 
    filter(region_id != 243)
}
keep <- c("surface", "ascent")
ml_filt_asgeir <- filt_ml("asgeir", keep)
```
