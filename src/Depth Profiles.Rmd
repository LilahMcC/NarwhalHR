...
one_dive$color <- with(one_dive, #assign dive phases to different time periods
                       ifelse(DateTime > des_start & DateTime <= des_end, "descent",
                              ifelse(DateTime > bot_start & DateTime <= bot_end, "bottom",
                                     ifelse(DateTime > asc_start & DateTime <= asc_end, "ascent",
                                            ifelse(DateTime > sub_start & DateTime <= sub_end, "subsurface", "surface"))))) %>% 
  factor(c("surface", "descent", "bottom", "ascent", "subsurface")) #reorder dive_phase legen labels to be in order of first appearance 
...
dive_plot <- ggplot(one_dive, aes(x = DateTime, y = Depth, color = color, group = 1)) +
# dive_plot <- ggplot(one_dive, aes(x = DateTime, y = Depth, color = color)) +
  ...
  scale_color_manual(values = c("#A6D854", "#FC8D62", "#77CCFF", "#E78AC3", "#8DA0CB")) 
  #scale_color_brewer(palette = "Set2")
dive_plot

#ggsave("figures/balder_dive_phases_fig.jpg", dive_plot, device="jpeg", width = 10, height = 7)
```
