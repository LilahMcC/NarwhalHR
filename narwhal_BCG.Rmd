...
```{r narwhal-beats}
#should turn this into a function that takes paths to acc files and motionless df, win_s, acc_fs , 
narwhal_beats <- function(ml_df, win_s, acc_fs){
  ...
  narwhal_100hz <- read_acc("HDI0_Asgeir.txt", 
                            ...
                            "HDK0_Asgeir.txt", 
                            nrow = Inf) %>% 
    mutate(retain = should_retain(DateTime, ml_all_asgeir, 2)) %>% 
    filter(retain) %>% 
    mutate(
    ...
    mutate(
      ...
      # Annotate regions
      rid_left = approx(ml_all_asgeir$time_start, 
                        ml_all_asgeir$region_id, 
                        DateTime, 
                        "constant")$y,
      rid_right = approx(ml_all_asgeir$time_end, 
                         ml_all_asgeir$region_id, 
                         DateTime, 
                         "constant", 
                         yleft = 0)$y + 1,
...
narwhal_beats(ml_best_asgeir, 2, 100)
...
```{r plot}
ggplot(narwhal_100hz, aes(x = jerk_smooth)) +
  geom_density() #plots a density plot to evaluate data 
```
...
```{r}
run_bcg <- function(win_size, upper) {
  
}
some_function <- function(win_size, upper) {
  do_all_the_bcg_stuff(with, win_size, and, upper)
  calculate_psd()
  calculate_s2n() # <- this is your return value
}
...
s2ns <- expand_grid(
  ...
  mutate(s2n = map2_dbl(win_size, upper, some_function))
```
