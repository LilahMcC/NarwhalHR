...
narwhal_100hz <- narwhal_beats(ml_best_asgeir, win_s = 1, acc_fs = 100, upper = 25.0) #should rid = NA in some cases?
...
quality_check <- function(win_size, upper) {
  narwhal_beats(ml_best_asgeir, win_s, acc_fs = 100, upper)
  calculate_psd()
...
s2ns <- expand_grid(
  win_s = win_sizes,
  upper = uppers) %>% 
  mutate(s2n = map2_dbl(win_s, upper, quality_check(win_s, upper)))
```
