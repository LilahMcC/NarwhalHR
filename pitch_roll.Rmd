...
acc_i <- read_tsv("HDI0_Asgeir.txt", col_names = col_name_vector, n_max = 100000) %>% 
  mutate(DateTime = mdy_hms(`DateTime`), .keep = "unused")   #fix datetime
# mutate(acc_i, acceleration = acceleration/1000, .keep = "unused")

acc_j <- read_tsv("HDJ0_Asgeir.txt", col_names = col_name_vector, n_max = 100000) %>% 
  mutate(DateTime = mdy_hms(`DateTime`), .keep = "unused") #fix datetime


acc_k <- read_tsv("HDK0_Asgeir.txt", col_names = col_name_vector, n_max = 100000) %>% 
  mutate(DateTime = mdy_hms(`DateTime`), .keep = "unused") #fix datetime
```
